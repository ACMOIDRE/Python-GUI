# Mini Project on GUI and Database
import tkinter
import tkinter.ttk
...
class Database:
    def __init__(self):
        self.dbConnection = sqlite3.connect("dbFile.db")    # create connection() to Database_file.db
        self.dbCursor = self.dbConnection.cursor()          # create cursor() for connection
    ...
    def __del__(self):
        # need to close cursor() and Connection() after modification eg: del,update,insert
        self.dbCursor.close()
        self.dbConnection.close()

    def Insert(self, id, fName, lName, dob, mob, yob, gender, address, phone, email, bloodGroup, history, doctor):
        #INSERT Values using tuples args(?,?,....etc),(id,fName,.....) can be done using dictionary also  ### dbcursor.execute(  "INSERT INTO patient_info VALUES (:first, :last, )",   { 'first': emp.first, 'last': emp.last, }  )
    ...
    def Update(self, fName, lName, dob, mob, yob, gender, address, phone, email, bloodGroup, history, doctor, id):   # SET statement is updater in sql
    ...
    def Search(self, id):  # Search is based on id only because id is PRIMARYKEY of database which Unique in database
...
######################   Number
Python, isdigit() is a built-in method used for string handling. The isdigit() methods returns “True” if all characters in the string are digits.
...
class Values:
    def Validate(self, id, fName, lName, phone, email, history, doctor):
        if not (id.isdigit() and (len(id) == 3)):                           # id start form 001 AND should equal to len XXX digits i.e, is 3
            return "id"
...
class InsertWindow:
    def __init__(self):
        ...
        self.yearList = list(range(1950, 2020))
